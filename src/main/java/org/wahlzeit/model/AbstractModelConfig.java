...
public abstract class AbstractModelConfig extends AbstractConfig implements ModelConfig,
   ...
   *
   */
  protected void initialize(Language myLanguage, DateFormat myDateFormatter,
      DecimalFormat myPraiseFormatter) {
    language = myLanguage;
    ...
    try {
      ...
      String shortDefaultFileName =
          myLanguage.asIsoCode() + File.separator + "ModelConfig.properties";
      if (templatesDir.hasDefaultFile(shortDefaultFileName)) {
        String absoluteDefaultFileName = templatesDir
            .getAbsoluteDefaultConfigFileName(shortDefaultFileName);
        loadProperties(absoluteDefaultFileName);
      }

      String shortCustomFileName =
          myLanguage.asIsoCode() + File.separator + "CustomModelConfig.properties";
      if (templatesDir.hasCustomFile(shortCustomFileName)) {
        String absoluteCustomFileName = templatesDir
            .getAbsoluteCustomConfigFileName(shortCustomFileName);
        loadProperties(absoluteCustomFileName);
      }
    } catch (IOException ioex) {
      log.warning(
          LogBuilder.createSystemMessage().addException("initializing directories failed", ioex)
              .toString());
    }
    ...
    loginMenu = loginMenu
        .replace("$loginPageLink$", userService.createLoginURL("/" + PartUtil.LOGIN_FORM_NAME));
    String guestMenu = baseMenu + menuDash + loginMenu;
    doSetValue("GuestMenu", guestMenu);

    String userMenu = doGetValue("UserMenuPart");
    userMenu = userMenu
        .replace("$logoutPageLink$", userService.createLogoutURL("/" + PartUtil.LOGOUT_PAGE_NAME));
