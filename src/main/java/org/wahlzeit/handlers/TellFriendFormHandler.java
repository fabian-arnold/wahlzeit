...
public class TellFriendFormHandler extends AbstractWebFormHandler {
  ...
  protected void doMakeWebPart(UserSession us, WebPart part) {
    ...
    part.maskAndAddStringFromArgsWithDefault(args, EMAIL_SUBJECT,
        config.getTellFriendEmailSubject());
  ...
  protected String doHandlePost(UserSession us, Map args) {
    ...
    emailService
        .sendEmailIgnoreException(to, config.getAuditEmailAddress(), emailSubject, emailBody);

    log.info(LogBuilder.createUserMessage().
        addAction("TellFriend").
        addParameter("recipient", to.asString()).toString());

    us.setTwoLineMessage(config.getEmailWasSent() + friendsEmailAddress + "! ",
        config.getKeepGoing());
