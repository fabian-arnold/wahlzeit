...
public abstract class AbstractWebPartHandler implements WebPartHandler {
  ...
  protected final WebPart createWebPart(UserSession us, String name) {
    ...
    WebPartTemplate tmpl = wpts
        .getTemplate(us.getClient().getLanguageConfiguration().getLanguageCode(), name);
  ...
  protected String getPhotoThumb(UserSession us, Photo photo) {
    ...
    if (photo != null) {
      ...
      result = HtmlUtil
          .asImg(HtmlUtil.asPath(imageLink), photo.getThumbWidth(), photo.getThumbHeight());
    } else {
  ...
  protected String getPhotoAsRelativeResourcePathString(Photo photo, PhotoSize size) {
    return SysConfig.getPhotosDir().getRelativeDir() + "/?type=image&photoId=" + photo.getId()
        .asString() +
